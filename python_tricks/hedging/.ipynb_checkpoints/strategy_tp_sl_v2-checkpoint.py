...
class HybridInternalTPSSL:
    ...
    def on_time(self, now: pd.Timestamp, state: Dict[str, Any]) -> List[Tuple[str, float]]:
        ...
        # --- Price-triggered TP/SL even without trades ---
        for inst, inv in state["inventory"].items():
            if inv == 0:
                continue
            mid = state.get("last_mid", {}).get(inst, None)
            if mid is None:
                continue
    
            # Tolerance band: skip hedging when small
            if self.p.tolerance_band_notional > 0:
                notional = abs(inv) * mid
                if notional <= self.p.tolerance_band_notional:
                    continue
            ...
            if not self._cooldown_ok(inst, now):
                continue
    ...
    def on_time(self, now, state):
        actions = []
    
        # --- 在没有成交的价格tick上也检查 TP/SL ---
        for inst, inv in state["inventory"].items():
            if inv == 0:
                continue
            mid = state.get("last_mid", {}).get(inst, None)
            ...
            if hedge_fraction > 0:
                qty = inv * hedge_fraction
                if abs(qty) >= self.p.min_trade_qty:
                    actions.append((inst, qty))
                    self.last_hedge_ts[inst] = now
    
        # --- 可选：到 EOD 清仓 ---
        if getattr(self, "eod_time", None) is not None and now >= self.eod_time and not state.get("_tp_sl_eod_done", False):
            state["_tp_sl_eod_done"] = True
            for inst, inv in state["inventory"].items():
                if inv != 0:
                    actions.append((inst, inv))
